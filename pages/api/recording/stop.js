...
// Google Cloud Storage Configuration (Firebase Storage)
const GOOGLE_CLOUD_STORAGE_BUCKET = process.env.GOOGLE_CLOUD_STORAGE_BUCKET || process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Stop the recording
    const stopResponse = await stopRecording(recordingInfo.resourceId, recordingInfo.sid, channelId);

    if (stopResponse.success) {
      // Update recording status in Firebase
      await updateRecordingInfo(meetingCode, {
        status: 'completed',
        endTime: Date.now(),
        fileList: stopResponse.fileList || [],
        storageLocation: `gs://${GOOGLE_CLOUD_STORAGE_BUCKET}/recordings/${meetingCode}/`
      });

      // Send notification email (optional)
      if (userRole && userRole !== 'admin') {
        await sendRecordingNotification(meetingCode, stopResponse.fileList);
      }
...
async function stopRecording(resourceId, sid, channelId) {
  ...
  if (response.ok) {
    return {
      ...
      message: 'Recording stopped successfully',
      fileList: data.fileList || [] // Assuming fileList is returned in the response
    };
  } else {
    return {
      success: false,
...
async function updateRecordingInfo(meetingCode, updateData) {
  try {
    const recordingRef = db.collection('Recordings').doc(meetingCode);
...
async function sendRecordingNotification(meetingCode, fileList) {
  try {
    // Get meeting details
    ...
    const emailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <p><strong>Durata înregistrării:</strong> ${Math.round((fileList[0]?.endTime - fileList[0]?.startTime) / 60000)} minute</p>
        </div>
