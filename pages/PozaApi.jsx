...
import PozaApiFields from "../components/Dashboard/PozaApiFields";

export default function PozaApi() {
  // const { db } = useMockup();
  ...
  const handleGetData = async () => {
    setIsLoading(true);
    const data = await getData("Others", "PozaApi");
  ...
  const confirmDelete = () => {
    ...
    const dataRef = ref(database, "Others/PozaApi/" + dialogData.id);
    remove(dataRef);
    ...
    const dbRef = ref(database, "Others/PozaApi/");
    set(dbRef, {}).then(() => {
    ...
    try {
      ...
      const updateData = db.map(async (item) => {
        if (item.id === dialogData.id) {
          ...
          editData(data, "Others", "Carti", dialogData.id);
          return data;
        } else {
  ...
  const handleUpload = async (info, selectedImages) => {
    try {
      ...
      await writeData(data, "Others", "Carti");
