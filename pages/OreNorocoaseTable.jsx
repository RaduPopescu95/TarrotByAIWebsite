...
import NumereNorocoaseFields from "../components/Dashboard/NumereNorocoaseFields";
import OreNorocoaseFields from "../components/Dashboard/OreNorocoaseFields";

export default function OreNorocoaseTable() {
  // const { db } = useMockup();
  ...
  const handleGetData = async () => {
    setIsLoading(true);
    const data = await getData("Others", "Ore-Norocoase");
  ...
  const confirmDelete = () => {
    ...
    const dataRef = ref(database, "Others/Ore-Norocoase/" + dialogData.id);
    remove(dataRef);
    ...
    const dbRef = ref(database, "Others/Ore-Norocoase/");
    set(dbRef, {}).then(() => {
  ...
  const handleEdit = async (info, ora) => {
    console.log("info....");
    ...
    try {
      ...
      const updateData = db.map(async (item) => {
        if (item.id === dialogData.id) {
          ...
          editData(data, "Others", "Ore-Norocoase", dialogData.id);
          return data;
        } else {
  ...
  const handleUpload = async (info, ora) => {
    try {
      const dateTime = getCurrentDateTime();
      ...
      await writeData(data, "Others", "Ore-Norocoase");
            ...
            <Box>
              ...
              <Stack direction="column" alignItems="center">
                {isLoading ? (
                  ...
                  >
                    Nu sunt ore norocoase adăugate
                  </Typography>
                ) : (
