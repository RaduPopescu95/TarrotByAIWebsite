...
function formatBillingInterval(interval, intervalCount) {
  ...
  return count === 1 ? `/ ${labels[interval] || interval}` : `/ ${count} ${labels[interval] || interval}`;
}
...
function DuplicateSubscriptionsSection({ report, loading, error, onRetry }) {
  ...
  return (
    <section
      className={`mb-8 overflow-hidden rounded-2xl border bg-white shadow-sm ${
        hasDuplicates ? "border-amber-300" : "border-slate-200"
      }`}
      aria-labelledby="duplicate-subscriptions-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h2 id="duplicate-subscriptions-title" className="text-base font-semibold text-slate-900">
              Abonamente Stripe duplicate
            </h2>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Grupare exactă după UID-ul Firebase; numele și emailul nu sunt folosite pentru asociere.
          </p>
        </div>
        {!loading && !error && report ? (
          <div className="flex flex-wrap gap-2 text-xs">
            <span className="rounded-full bg-amber-100 px-2.5 py-1 font-semibold text-amber-900">
              {report.totalUsers || 0} {(report.totalUsers || 0) === 1 ? "utilizator" : "utilizatori"}
            </span>
            <span className="rounded-full bg-rose-100 px-2.5 py-1 font-semibold text-rose-800">
              {report.extraSubscriptions || 0} suplimentare
            </span>
          </div>
        ) : null}
      </div>
      ...
      {loading ? (
        <div className="space-y-3 p-5" aria-label="Se verifică abonamentele Stripe">
        ...
        <div className="m-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          <p>Nu am putut verifica abonamentele duplicate: {error}</p>
          <button
            type="button"
            onClick={onRetry}
        ...
        <div className="m-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-900">
        ...
        <div className="space-y-4 p-5">
          {groups.map((group) => {
            ...
            return (
              <article key={group.uid} className="overflow-hidden rounded-xl border border-amber-200 bg-amber-50/40">
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-amber-100 px-4 py-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-900">{fullName || group.email || "Utilizator fără nume"}</p>
                    {group.email ? (
                      <a href={`mailto:${group.email}`} className="text-sm text-indigo-700 hover:underline">
                        {group.email}
                      </a>
                    ) : null}
                    <p className="mt-1 break-all font-mono text-[11px] text-slate-500" title="Firebase UID">
                      UID: {group.uid}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs">
                  ...
                  <table className="min-w-full divide-y divide-slate-100">
                    <thead className="bg-slate-50/80">
                      <tr>
                        {["Abonament", "Customer", "Status", "Creat", "Perioada curentă", "Preț", "Anulare"].map((label) => (
                          <th key={label} className="whitespace-nowrap px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                            {label}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {group.subscriptions.map((subscription) => (
                        <tr key={subscription.id} className="text-sm text-slate-700">
                          ...
                          <td className="whitespace-nowrap px-3 py-3">{formatDate(subscription.createdAt)}</td>
                          <td className="whitespace-nowrap px-3 py-3">
                          ...
                          <td className="whitespace-nowrap px-3 py-3">
                            ...
                            {subscription.priceId ? (
                              <p className="mt-1 font-mono text-[10px] text-slate-400" title={subscription.productId || undefined}>
                                {subscription.priceId}
                              </p>
                            ) : null}
                          </td>
      ...
      {!loading && !error && report?.unresolvedCount > 0 ? (
        <div className="border-t border-slate-100 bg-slate-50 px-5 py-3 text-xs text-amber-900">
...
function SubscribersScreen() {
  ...
  const loadDuplicates = useCallback(async () => {
    ...
    setDuplicateError("");
    try {
      const res = await fetch("/api/dashboard/subscription-duplicates", {
        cache: "no-store",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || "duplicate_load_failed");
      setDuplicateReport(data);
    } catch (e) {
      setDuplicateError(e?.message || "Eroare la verificarea Stripe");
    } finally {
  ...
  const load = useCallback(async () => {
    await Promise.all([loadSubscribers(), loadDuplicates()]);
  }, [loadDuplicates, loadSubscribers]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-7">
          ...
          <StatCard
            label="UID-uri duplicate"
            value={duplicateLoading ? "…" : duplicateError ? "!" : duplicateReport?.totalUsers || 0}
            sub={duplicateError ? "verificare eșuată" : `${duplicateReport?.extraSubscriptions || 0} suplimentare`}
            color={duplicateReport?.totalUsers > 0 ? "border-rose-300" : "border-emerald-200"}
          />
        </div>

        <DuplicateSubscriptionsSection
