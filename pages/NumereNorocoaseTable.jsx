...
export default function NumereNorocoaseTable() {
  // const { db } = useMockup();
  ...
  const handleGetData = async () => {
    setIsLoading(true);
    const data = await getData("Others", "Numere-Norocoase");
  ...
  const confirmDelete = () => {
    ...
    const dataRef = ref(database, "Others/Numere-Norocoase/" + dialogData.id);
    remove(dataRef);
    ...
    const dbRef = ref(database, "Others/Numere-Norocoase/");
    set(dbRef, {}).then(() => {
  ...
  const handleEdit = async (info, number) => {
    console.log("info....");
    ...
    try {
      ...
      const updateData = db.map(async (item) => {
        if (item.id === dialogData.id) {
          ...
          editData(data, "Others", "Numere-Norocoase", dialogData.id);
          return data;
        } else {
  ...
  const handleUpload = async (info, number) => {
    try {
      const dateTime = getCurrentDateTime();
      ...
      await writeData(data, "Others", "Numere-Norocoase");
            ...
            <Box>
              ...
              <Stack direction="column" alignItems="center">
                {isLoading ? (
                  ...
                  >
                    Nu sunt numere norocoase adăugate
                  </Typography>
                ) : (
