...
function findRelatedArticlesFromRows({ rows, categoryKey, excludeDocumentId, limit, nowMs }) {
  ...
  return refreshArticleRowsSchedule(rows)
    .filter((row) => normalizeString(row.categoryKey || extractCategoryKey(row)) === normalizedCategory)
    .filter((row) => isArticlePublishedAt(row, nowMs))
    .filter((row) => normalizeString(row.documentId) !== excludeId)
    .slice(0, limit);
}

async function loadArticleRowsForDerivedData() {
  const rows = await loadPublicArticleRows();
  return refreshArticleRowsSchedule(rows).filter((row) => {
    const scheduledAtMs = Number(row?.scheduledAtMs);
...
export async function loadPublicArticles({
  ...
  if (normalizedId) {
    const [found, derivedRows] = await Promise.all([
      resolveArticleById(normalizedId),
      loadArticleRowsForDerivedData(),
    ]);
    const nextPublishAtMs = findNextPublishAtMs(derivedRows, nowMs);
    if (!found || !isArticlePublishedAt(found, nowMs)) {
      return {
        articles: [],
        items: [],
...
export async function loadPublicArticleDetail({
  ...
  const [articleRow, derivedRows] = await Promise.all([
    resolveArticleById(normalizedId),
    loadArticleRowsForDerivedData(),
  ]);
  const nextPublishAtMs = findNextPublishAtMs(derivedRows, nowMs);
  ...
  const relatedRows = findRelatedArticlesFromRows({
    rows: derivedRows,
    categoryKey,
