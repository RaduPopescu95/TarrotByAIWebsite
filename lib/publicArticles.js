...
const dedupeById = (articles) => {
  ...
  return articles.filter((article) => {
    const id = String(article?.id || article?.documentId || "").trim();
...
export async function loadPublicArticles({
  ...
  const loadLegacyCandidates = async () => {
    ...
    return legacySnap.docs
      .map(docToArticle)
      .filter((article) => resolveArticleSortDate(article) <= now);
  };
  ...
  if (normalizedSearch) {
    ...
    const [snap, legacyArticles] = await Promise.all([
    ...
    let articles = dedupeById([
      ...snap.docs.map(docToArticle),
      ...legacyArticles,
    ]).filter((article) =>
