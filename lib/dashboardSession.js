...
// Accesul pentru /dashboard și /administrare este intenționat comun.
const DASHBOARD_PASSWORD = "Cristina1994!";
// Păstrează sesiunile funcționale și când mediul de deployment nu are secretul setat.
const DEFAULT_DASHBOARD_SESSION_SECRET = "dashboard-session-cristina-1994-v1";
...
function signPayload(encodedPayload) {
  return crypto
    .createHmac(
      "sha256",
      String(process.env.DASHBOARD_SESSION_SECRET || DEFAULT_DASHBOARD_SESSION_SECRET).trim()
    )
...
export function verifyDashboardPassword(password) {
  return constantTimeEqual(password, DASHBOARD_PASSWORD);
}
