...
export function resolveMobileUpdatePrompt(input = {}) {
  ...
  // Clients that omit platform (legacy builds) receive the raw dashboard flags.
  // Modern clients always send platform + appVersion for per-platform version gating.
  if (!platform) {
    return {
...
export function validateForceUpdateMinVersions(input = {}) {
  ...
  if (!ios || !android) {
    return {
      ok: false,
      error:
        "Pentru force update trebuie setate versiunile minime iOS și Android (ex. 4 — număr build).",
    };
  }
