...
function filterPlayableVideoRows(sortedRows, localeNorm, nowMs, { excludeIds = null } = {}) {
...
export async function loadFeaturedHomeVideos({
  ...
  const featuredRows = filterPlayableVideoRows(sortedRows, localeNorm, nowMs).filter(
  ...
  const excluded =
    excludeIds instanceof Set
      ...
      : Array.isArray(excludeIds)
        ...
        : null;
  let playable = filterPlayableVideoRows(sortedRows, localeNorm, nowMs, { excludeIds: excluded });
