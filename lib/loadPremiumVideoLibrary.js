...
async function rebuildMaterializedRows(db) {
  ...
  const manifestPayload = {
    ...
    chunkDocIds,
    featuredVideoIds: sortedRows
      .filter((row) => row.featuredOnHome === true)
      .map((row) => row.id),
  };
...
async function fetchChunkRowsFromManifest(db, manifest) {
  ...
  const featuredVideoIds = Array.isArray(manifest?.featuredVideoIds)
    ? [...new Set(
        manifest.featuredVideoIds.filter(
          (id) => typeof id === "string" && id.trim()
        ).map((id) => id.trim())
      )]
    : null;
  if (
    manifest?.version !== PUBLIC_CACHE_VERSION ||
    chunkDocIds.length === 0 ||
    featuredVideoIds === null
  ) {
    return null;
  }
  ...
  const loadedIds = new Set(rowsFromChunks.map((row) => row.id));
  const missingFeaturedIds = featuredVideoIds.filter((id) => !loadedIds.has(id));
  if (missingFeaturedIds.length === 0) {
    return rowsFromChunks;
  }

  const featuredSnaps = await Promise.all(
    missingFeaturedIds.map((videoId) =>
      withFirestoreCostLog(
        {
          page: "api.video-library",
          queryName: "videos.featured_outside_loaded_chunks",
          operationType: "document",
        },
        () => db.collection(COLLECTION).doc(videoId).get()
      )
    )
  );
  const missingFeaturedRows = featuredSnaps
    .map(normalizePublishedVideoRow)
    .filter(Boolean);

  return sortVideoDocsForPublic([...rowsFromChunks, ...missingFeaturedRows]);
}
