...
export function toSafeCourseBundle(bundleId, data = {}, locale = "ro", courses = []) {
  ...
  const displayPricing = applyFixedVatDisplayPrice({
  ...
  return {
    ...
    priceIncludesVat: displayPricing.priceIncludesVat,
    vatPercentage: displayPricing.vatPercentage,
    ...
    purchaseCount:
      typeof data.purchaseCount === "number" && Number.isFinite(data.purchaseCount)
        ...
        : 0,
    courses: normalizedCourses.map((course) => applyFixedVatDisplayPrice(course)),
