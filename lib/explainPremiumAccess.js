...
import {
  hasActivePremiumSource,
  hasCanonicalPremiumSources,
...
export function buildPremiumAccessSnapshot(userData) {
  ...
  return {
    ...
    activePremiumProviders: Array.isArray(userData.activePremiumProviders)
      ? userData.activePremiumProviders.filter((value) => typeof value === "string")
      : [],
    premiumSources:
...
export function explainPremiumAccess(userData) {
  ...
  if (hasCanonicalPremiumSources(userData)) {
    return hasActivePremiumSource(userData.premiumSources)
      ? { hasAccess: true, reason: "provider_source_active", snapshot, now }
      : { hasAccess: false, reason: "provider_sources_inactive", snapshot, now };
  }
  const inferredSources = inferLegacyPremiumSources(userData);
  const inferredKeys = Object.keys(inferredSources);
  if (inferredKeys.length > 1 || inferredKeys[0] === "revenuecat") {
    return hasActivePremiumSource(inferredSources)
      ? { hasAccess: true, reason: "legacy_provider_source_active", snapshot, now }
      : { hasAccess: false, reason: "legacy_provider_sources_inactive", snapshot, now };
  }
  if (inferredKeys[0] === "stripe" && !readStatus(userData)) {
    return { hasAccess: false, reason: "legacy_provider_sources_inactive", snapshot, now };
  }
