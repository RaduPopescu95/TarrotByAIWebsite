...
export async function softDeleteAnalysesForIdentity(identity) {
  assertUsableIdentity(identity);
  const db = getAdminDb();
  let softDeleted = 0;

  for (const family of Object.keys(ANALYSIS_FAMILIES)) {
    const items = await queryFamilyByIdentity(db, family, identity);
    ...
    items.forEach((item) => {
      ...
      batch.set(
        db.collection(ANALYSIS_FAMILIES[family]).doc(item.documentId),
        {
          isActive: false,
          deletedAt: FieldValue.serverTimestamp(),
          updatedAt: FieldValue.serverTimestamp(),
