...
async function loadPublicTarotManifest(db) {
  ...
  if (isManifestMemoryFresh(now)) {
    recordFirestoreCacheHit({
      page: "api.public-tarot",
      queryName: "memory.tarot_manifest",
    });
  ...
  if (!manifestInflight.promise) {
    manifestInflight.promise = (async () => {
      const manifestSnap = await withFirestoreCostLog(
        {
          page: "api.public-tarot",
          queryName: "internalCaches.tarot_manifest",
          operationType: "document",
        },
        () => db.collection(CACHE_COLLECTION).doc(MANIFEST_DOC_ID).get()
      );
...
async function loadRowsForManifestEntry(db, manifest, category, key) {
  ...
  const chunkSnaps = await Promise.all(
    chunkDocIds.map((docId) =>
      withFirestoreCostLog(
        {
          page: "api.public-tarot",
          queryName: "internalCaches.tarot_chunk",
          operationType: "document",
        },
        () => db.collection(CACHE_COLLECTION).doc(docId).get()
      )
    )
  );
...
async function persistMaterializedCache(datasetMap) {
  ...
  const existingManifestSnap = await withFirestoreCostLog(
    {
      page: "api.public-tarot.rebuild",
      queryName: "internalCaches.tarot_manifest_existing",
      operationType: "document",
    },
    () => manifestRef.get()
  );
...
export async function loadPublicTarotDataset(category, key) {
  ...
  if (isMemoryFresh(memoryEntry)) {
    recordFirestoreCacheHit({
      page: "api.public-tarot",
      queryName: "memory.tarot_dataset",
    });
    return {
      arr: memoryEntry.arr,
      source: "MEMORY",
