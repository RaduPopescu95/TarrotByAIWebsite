...
/**
 * Removes paid-course detail content for callers without entitlement.
 * Client-side flags (e.g. legacy iOS free mode) cannot override this response.
 *
 * The preview teaser (previewVimeoId / thumbnailUrl) is intentionally kept: it is
 * public marketing material, not the paid lessons. Course cards and the locked
 * detail view need it to render a thumbnail. The actual course video stays gated
 * by the entitlement check on /api/courses/[id]/playback, so withholding only the
 * paid textual content (curriculum/notes/contact) is enough here.
 */
export function applyCourseDetailContentGate(safeCourse, hasAccess) {
  if (!safeCourse || hasAccess === true) {
    return safeCourse;
  }

  const price = typeof safeCourse.price === "number" ? safeCourse.price : 0;
