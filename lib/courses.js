...
export const COURSE_CHANNELS = ["website", "mobile"];

export function normalizeCourseChannel(value, fallback = "mobile") {
  if (value === "website" || value === "mobile") return value;
  return fallback;
}

/**
 * Legacy Expo builds send no client marker, so an unmarked request must remain
 * mobile. The website opts in explicitly through `channel=website`.
 */
export function resolveCourseRequestChannel(req) {
  const queryChannel = readSingleQueryValue(req?.query?.channel);
  if (queryChannel === "website" || queryChannel === "mobile") {
    return queryChannel;
  }
...
export function resolveCourseChannelFromPlatform(value) {
  const normalized = typeof value === "string" ? value.trim().toLowerCase() : "";
  return normalized === "expo" ||
    normalized === "mobile" ||
...
export function isCourseAvailableOnChannel(course, channel) {
  if (!course) return false;
  const normalizedChannel = normalizeCourseChannel(channel);
  if (normalizedChannel === "website") return course.availableOnWebsite !== false;
  return course.availableOnMobile !== false;
}

export function hasAtLeastOneCourseChannel(course) {
  if (!course) return false;
  return course.availableOnWebsite !== false || course.availableOnMobile !== false;
}
...
export function isCourseVisibleOnChannel(course, channel, nowMs = Date.now()) {
  return isCourseVisible(course, nowMs) && isCourseAvailableOnChannel(course, channel);
}
...
export function applyFixedVatDisplayPrice(safeCourse) {
  if (!safeCourse || typeof safeCourse !== "object") return safeCourse;
  if (typeof safeCourse.price !== "number" || !Number.isFinite(safeCourse.price)) {
    return safeCourse;
  }
  return {
    ...safeCourse,
    price: calculateFixedVatMajor(safeCourse.price),
...
export function toSafeCourseForPublicCatalog(courseId, data = {}, locale = DEFAULT_LOCALE) {
  return applyFixedVatDisplayPrice(toSafeCourse(courseId, data, locale));
}
