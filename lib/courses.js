...
export function applyCourseDetailContentGate(safeCourse, hasAccess) {
  ...
  const gatedPreview = applyCoursePreviewAccessGate(safeCourse, false);
...
/**
 * Public list/home responses never include preview video IDs for paid courses.
 */
export function toSafeCourseForPublicCatalog(courseId, data = {}, locale = DEFAULT_LOCALE) {
  const safeCourse = toSafeCourse(courseId, data, locale);
  if (typeof safeCourse.price === "number" && safeCourse.price > 0) {
    return applyCoursePreviewAccessGate(safeCourse, false);
  }
  return safeCourse;
}
