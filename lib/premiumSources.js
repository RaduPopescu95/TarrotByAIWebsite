...
/**
 * True only when the profile contains durable evidence that a Google Play
 * premium subscription existed. An inactive placeholder written after an
 * empty RevenueCat restore is deliberately not purchase history.
 */
export function hasRevenueCatPurchaseEvidence(userData) {
  const source = userData?.premiumSources?.revenuecat;
  return Boolean(
    cleanString(source?.productId) ||
      cleanString(source?.originalTransactionId) ||
      currentPeriodEndToMillis(source?.expiresAt || source?.currentPeriodEnd) != null ||
      cleanString(userData?.revenueCatProductId) ||
      cleanString(userData?.revenueCatOriginalTransactionId) ||
      currentPeriodEndToMillis(userData?.revenueCatCurrentPeriodEnd) != null
  );
}

export function getRevenueCatLifecycle(userData, nowMs = Date.now()) {
  const source = userData?.premiumSources?.revenuecat;
  const hasPurchaseHistory = hasRevenueCatPurchaseEvidence(userData);
  ...
  const expiresAt =
    ...
    userData?.revenueCatCurrentPeriodEnd ??
    null;
  const expiresMs = currentPeriodEndToMillis(expiresAt);
  const active =
    source?.active === true && (expiresMs == null || expiresMs > nowMs);
  const sourceStatus = cleanString(
    source?.status || userData?.revenueCatSubscriptionStatus
  ).toLowerCase();
  const cancelAtPeriodEnd =
    active &&
    (source?.cancelAtPeriodEnd === true ||
      source?.willRenew === false ||
      userData?.revenueCatCancelAtPeriodEnd === true);

  let status = "expired";
  if (active && cancelAtPeriodEnd) status = "cancel_at_period_end";
  else if (active && ["past_due", "billing_issue", "unpaid"].includes(sourceStatus)) {
    status = "past_due";
