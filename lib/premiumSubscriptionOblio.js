...
import { getOblioVatSettings, resolveStripeInvoiceOblioTax, shouldSendOblioEInvoice } from "../utils/oblioTax";
...
/** Build an individual billing snapshot for legacy subscriptions from the finalized Stripe Invoice. */
export function buildStripeInvoiceFallbackBilling(invoice) {
  const parsedName = splitName(invoice?.customer_name || "");
  const address = normalizeAddress(invoice?.customer_address || invoice?.customer_shipping?.address);
  const hasCountry = Boolean(address.country);
  address.country = normalizeCountry(address.country);
  const email = sanitizeString(invoice?.customer_email, 320);
  const isRomania = address.country === "Romania";
  ...
  return {
    ...
    email,
    phone: sanitizeString(invoice?.customer_phone, 64),
    cnp: "",
    address,
    company: { name: "", vat: "", reg: "", address: "" },
    invoicePreferences: { sendEmail: true, eInvoice: false, dueDays: null },
  };
}
...
export async function emitPremiumSubscriptionOblioInvoice({ db, stripe, invoice, premiumFlowMetadata }) {
  ...
  const storedProfile = userSnap.exists ? userSnap.data()?.premiumBillingProfile : null;
  const fallbackBilling = buildStripeInvoiceFallbackBilling(invoice);
  const profile =
    storedProfile?.billing && typeof storedProfile.billing === "object"
      ? storedProfile
      : fallbackBilling
        ? {
            billing: fallbackBilling,
            rawFormValues: fallbackBilling,
            stripeMetadataSnapshot: subscription.metadata || {},
          }
        : null;
  if (!profile) {
    ...
    customer_details:
      invoice.customer_email || invoice.customer_name
        ? {
            ...
            address: invoice.customer_address || invoice.customer_shipping?.address || null,
  ...
  const taxLine = resolveStripeInvoiceOblioTax(invoice, getOblioVatSettings());
  if (!taxLine.ok) {
    console.error("[premium.oblio] blocked_total_mismatch", { invoiceId, reason: taxLine.reason, amountPaid });
    await oblioRef.set({ status: "blocked_total_mismatch", invoiceId, uid, subscriptionId: subId, reason: taxLine.reason, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    return { skipped: true, reason: "blocked_total_mismatch", invoiceId };
  }
