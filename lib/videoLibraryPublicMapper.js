...
/** YouTube/Vimeo: treat as public playback even if isPremium when created strictly before 2026-05-02 00:00 UTC (incl. everything on/before 2026-05-01). Uses updatedAt only if createdAt is missing. */
const PREMIUM_WORKAROUND_FREE_BEFORE_MS = Date.UTC(2026, 4, 2, 0, 0, 0);

function premiumWorkaroundTreatAsFree(row, platformNorm) {
  if (platformNorm !== "youtube" && platformNorm !== "vimeo") return false;
  const ms =
    firestoreTsToMillis(row.createdAt) ?? firestoreTsToMillis(row.updatedAt);
  if (ms == null) return false;
  return ms < PREMIUM_WORKAROUND_FREE_BEFORE_MS;
}
