import { hasActivePremiumSource, inferLegacyPremiumSources } from "./premiumSources";
...
export function hasPremiumAccess(userData) {
  ...
  if (hasActivePremiumSource(userData.premiumSources)) return true;
...
/**
 * Dashboard manual grant (`subscriptionProvider: "manual"`) with active access — Stripe sync must not overwrite.
 * @param {object|null} userData
 */
export function isManualPremiumProtected(userData) {
  if (!userData || typeof userData !== "object") return false;
  const manualSource = inferLegacyPremiumSources(userData).manual;
  if (manualSource?.active === true) {
    const manualEnd = currentPeriodEndToMillis(
      manualSource.expiresAt || manualSource.currentPeriodEnd
    );
    return manualEnd == null || Date.now() < manualEnd;
  }
  if (userData.subscriptionProvider !== "manual") return false;
