...
function extractAdsterraContainerId(scriptSrc) {
  const raw = String(scriptSrc || "").trim();
  if (!raw) return "";
  const match = raw.match(/\/([a-f0-9]{16,})\/invoke\.js(?:\?.*)?$/i);
  if (!match) return "";
...
export function getAdsEnv() {
  ...
  const adsterraContainerId =
    process.env.NEXT_PUBLIC_ADSTERRA_CONTAINER_ID ||
    extractAdsterraContainerId(adsterraScriptSrc);
