...
function resolveRoutePreferredProvider(pathname) {
  const normalized = normalizePath(pathname);
  if (normalized === "/") return AD_PROVIDERS.MONETAG;
  if (/^\/news(?:\/|$)/.test(normalized)) return AD_PROVIDERS.ADSTERRA;
...
export function resolveActiveAdProvider(pathname, adsEnv) {
  ...
  const preferredProvider = resolveRoutePreferredProvider(pathname);
