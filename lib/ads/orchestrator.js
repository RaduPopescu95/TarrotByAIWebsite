...
export function resolveActiveAdProvider(pathname, adsEnv) {
  ...
  if (adsEnv.primaryProvider === AD_PROVIDERS.ADSENSE && hasAdsense) {
    return AD_PROVIDERS.ADSENSE;
  }
  if (adsEnv.primaryProvider === AD_PROVIDERS.MONETAG && hasMonetag) {
    return AD_PROVIDERS.MONETAG;
  }
  if (adsEnv.primaryProvider === AD_PROVIDERS.ADSTERRA && hasAdsterra) {
    return AD_PROVIDERS.ADSTERRA;
  }

  if (hasAdsense) return AD_PROVIDERS.ADSENSE;
  if (hasMonetag) return AD_PROVIDERS.MONETAG;
  if (hasAdsterra) return AD_PROVIDERS.ADSTERRA;
  return AD_PROVIDERS.NONE;
}
