...
export function extractBunnyEmbedParts(videoUrl) {
  ...
  const configuredCdnHost = getBunnyStreamCdnHostname().toLowerCase();
  if (configuredCdnHost && host === configuredCdnHost) {
    const hlsMatch = url.pathname.match(
      /^\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:\/|$)/i
    );
    const libraryId = getDefaultBunnyLibraryId();
    if (hlsMatch && libraryId) {
      return { libraryId, videoId: hlsMatch[1] };
    }
  }
...
export function resolveBunnyHlsSrcFromVideoUrl(platform, videoUrl) {
  ...
  return `https://${cdnHost}/${parts.videoId}/playlist.m3u8`;
}
...
function appendUrlParams(url, entries) {
...
export function appendLibraryAutoplayParams(embedSrc, platform, locale = "ro") {
  ...
  if (normalizedPlatform === "youtube") {
    return appendUrlParams(embedSrc, [
      ["autoplay", 1],
      ["playsinline", 1],
      ["controls", 1],
      ["modestbranding", 1],
  ...
  return appendUrlParams(embedSrc, [["autoplay", 1]]);
}
...
/**
 * Resolve a playback URL while preserving compatibility with clients that cannot
 * switch locale or player implementation. Prefer the requested locale, then the
 * denormalized root, Romanian, and finally the first localized source.
 *
 * @param {object} row Firestore-shaped video doc
 * @param {string} localeNorm lowercase base locale code (e.g. "ro")
 * @returns {{ source: string; strategy: "requested_locale" | "root" | "ro_fallback" | "first_locale_fallback" | "missing"; sourceLocale: string | null }}
 */
export function resolveRowVideoSourceWithMeta(row, localeNorm) {
  const L =
  ...
  if (!hasAnyLocalizedVideoUrl(row)) {
    return {
      source: root,
      strategy: root ? "root" : "missing",
      sourceLocale: null,
    };
  }
  const locales = row.locales || {};
  const requested = locales[L]?.videoUrl;
  if (typeof requested === "string" && requested.trim()) {
    return {
      source: requested.trim(),
      strategy: "requested_locale",
      sourceLocale: L,
    };
  }
  if (root) {
  ...
  const romanian = locales.ro?.videoUrl;
  if (typeof romanian === "string" && romanian.trim()) {
    return {
      source: romanian.trim(),
      strategy: "ro_fallback",
      sourceLocale: "ro",
    };
  }
  for (const key of Object.keys(locales)) {
    const candidate = locales[key]?.videoUrl;
    if (typeof candidate === "string" && candidate.trim()) {
      return {
        source: candidate.trim(),
        strategy: "first_locale_fallback",
        sourceLocale: key,
      };
    }
  }
  return { source: "", strategy: "missing", sourceLocale: null };
}

/**
 * Raw playback URL string selected by the compatibility resolver.
 */
export function resolveRowVideoSource(row, localeNorm) {
  return resolveRowVideoSourceWithMeta(row, localeNorm).source;
}
...
/** True only when the requested locale itself has a playable source (no compatibility fallback). */
export function rowHasDirectValidEmbedForLocale(row, localeRaw) {
  const localeNorm = normalizeLocale(localeRaw, "ro");
  const platform = normalizeVideoPlatform(row.platform);
  const localized = row?.locales?.[localeNorm]?.videoUrl;
  const root = typeof row?.videoUrl === "string" ? row.videoUrl.trim() : "";
  const raw =
    typeof localized === "string" && localized.trim()
      ? localized.trim()
      : !hasAnyLocalizedVideoUrl(row)
        ? root
        : "";
  return Boolean(raw && resolveLibraryEmbedSrc(platform, raw));
}
