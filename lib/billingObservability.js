...
export function getSafeBillingConfigSnapshot(settings = {}) {
  ...
  const describe = (platform, flow, rolloutSetting, disabledValue = "stripe") => {
    const row = config[platform]?.[flow] || {};
    const rollout = settings?.[rolloutSetting] === "revenuecat"
      ? "revenuecat"
      : disabledValue;
  ...
  return {
    ...
    premium: {
      ...describe("android", "premium", "androidBillingPremiumProvider"),
      rollout: "revenuecat",
      selectedProvider: "revenuecat",
    },
    analyses: {
      ...describe("android", "analyses", "androidBillingAnalysesProvider"),
    ...
    platforms: {
      android: {
        ...
        analyses: describe("android", "analyses", "androidBillingAnalysesProvider"),
      },
      ios: {
        premium: describe(
          "ios",
          "premium",
          "iosBillingPremiumProvider",
          "disabled"
        ),
        analyses: {
          ...describe(
          ...
          configuredProductCount: Object.values(
            config.ios.analyses?.productIds || {}
