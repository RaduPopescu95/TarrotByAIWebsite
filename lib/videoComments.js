...
export const VIDEO_COMMENT_PLACEHOLDER_AUTHORS = ["Utilizator", "Guest", "User"];
...
function resolveCommentAuthorFirstName(author, clientFirstName) {
  const profileName = cleanString(author?.firstName, 80);
  const clientName = cleanString(clientFirstName, 80);
  if (
    clientName &&
    !VIDEO_COMMENT_PLACEHOLDER_AUTHORS.includes(clientName) &&
    (!profileName || VIDEO_COMMENT_PLACEHOLDER_AUTHORS.includes(profileName))
  ) {
    return clientName;
  }
  return profileName || "Utilizator";
}

async function persistUserFirstNameIfMissing(uid, firstName, db) {
  const normalized = cleanString(firstName, 80);
  if (!uid || !normalized || VIDEO_COMMENT_PLACEHOLDER_AUTHORS.includes(normalized)) {
    return;
  }
  await db.collection("Users").doc(uid).set(
    { owner_uid: uid, first_name: normalized },
    { merge: true }
  );
}
...
export async function createVideoComment({
  ...
  const resolvedFirstName = resolveCommentAuthorFirstName(author, clientAuthorFirstName);
  if (resolvedFirstName !== author.firstName) {
    await persistUserFirstNameIfMissing(uid, resolvedFirstName, db);
    author.firstName = resolvedFirstName;
  }
...
export async function syncCommentAuthorNamesForUser({
  uid,
  decoded = null,
  db = getAdminDb(),
}) {
  if (!uid) {
    return { updated: 0, reason: "missing_uid" };
  }
  const author = await loadCommentAuthor(uid, decoded, db);
  const realName = cleanString(author?.firstName, 80);
  if (!realName || VIDEO_COMMENT_PLACEHOLDER_AUTHORS.includes(realName)) {
    return { updated: 0, reason: "no_real_name" };
  }
  ...
  const snap = await db
    ...
    .get();
  if (snap.empty) {
    return { updated: 0, reason: "no_comments", name: realName };
  }

  const targets = [];
  snap.forEach((docSnap) => {
    const data = docSnap.data() || {};
    const currentName = cleanString(data.authorFirstName, 80);
    if (!currentName || VIDEO_COMMENT_PLACEHOLDER_AUTHORS.includes(currentName)) {
      targets.push(docSnap.ref);
    }
  });
  if (targets.length === 0) {
    return { updated: 0, reason: "already_synced", name: realName };
  }
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    ...
    for (const ref of targets.slice(i, i + BATCH_SIZE)) {
      batch.update(ref, {
        authorFirstName: realName,
        authorNameSyncedAt: FieldValue.serverTimestamp(),
      });
    }
    await batch.commit();
  }
  return { updated: targets.length, name: realName };
}
