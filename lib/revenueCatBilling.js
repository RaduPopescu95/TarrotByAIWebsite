...
export async function resolveRevenueCatProduct(db, rawProductId) {
  ...
  if (isRevenueCatFlowEnabled("analyses")) {
    ...
    if (ledgerMapping) {
      return { kind: "analysis", productId, analysisKey, ...ledgerMapping };
    }
  }
  ...
  if (isRevenueCatFlowEnabled("courses")) {
    ...
    if (staticCourseId) {
      return { kind: "course", productId, itemId: staticCourseId, source: "env" };
    }
    const staticBundleId = lookupStaticProduct(getStaticBundleProductMap(), productId);
    if (staticBundleId) {
      return { kind: "bundle", productId, itemId: staticBundleId, source: "env" };
    }
...
function buildPremiumWrite(userData, event, mapping) {
...
export async function processRevenueCatEvent(db, event) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (eventSnap.exists) return { skipped: true, reason: "already_processed" };
    ...
    if (mapping?.kind === "premium") {
      ...
      transaction.set(userRef, buildPremiumWrite(userData, event, mapping), { merge: true });
...
export async function syncRevenueCatPremiumFromSubscriber(db, uid, subscriber) {
  ...
  const active = Boolean(
    entitlement && (!Number.isFinite(expiresMs) || expiresMs > Date.now())
  );
  const userRef = db.collection("Users").doc(uid);
  return db.runTransaction(async (transaction) => {
    const snap = await transaction.get(userRef);
    const userData = snap.exists ? snap.data() || {} : {};
    const expiresAt = Number.isFinite(expiresMs) ? Timestamp.fromMillis(expiresMs) : null;
    const productId = clean(entitlement?.product_identifier);
    transaction.set(
      userRef,
      {
        ...buildPremiumSourcePatch(userData, "revenuecat", {
        ...
        revenueCatSubscriptionStatus: active ? "active" : "expired",
        revenueCatCurrentPeriodEnd: expiresAt,
    ...
    return { active, productId: productId || null };
  });
}
...
export async function fetchRevenueCatSubscriber(uid, fetchImpl = fetch) {
  ...
  if (!apiKey) throw new Error("RevenueCat API is not configured");
