  ...
  const snap = await withFirestoreCostLog(
    {
      ...
      queryName: "AfirmatiiPozitive.all",
      locale: "ro",
      isrRevalidateSeconds: 86400,
    },
    () => db.collection("AfirmatiiPozitive").get()
  );
  const allItems = snap.docs.map(docToObject).filter((item) => item?.info);
  shuffleInPlace(allItems);
  return allItems.slice(0, resolvedLimit);
}
