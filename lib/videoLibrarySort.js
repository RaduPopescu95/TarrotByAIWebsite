/**
 * Public video library ordering: newest published first.
 * Priority: publishAt desc (when set), then createdAt desc, then updatedAt desc.
...
function rowNewestFirstMs(row) {
  const publishAt = timestampToMs(row?.publishAt);
  if (publishAt > 0) return publishAt;
