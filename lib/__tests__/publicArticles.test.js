...
describe("publicArticles", () => {
  ...
  it("queries published articles directly without full collection scan", async () => {
    const now = Date.now();
    ...
    expect(db.fullCollectionGetCount()).toBe(0);
  });

  it("paginates published articles with Firestore cursor", async () => {
    const now = Date.now();
    const db = buildQueryDb({
      articles: [
        {
