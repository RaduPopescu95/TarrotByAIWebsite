...
const buildDb = ({ manifest, chunks = {}, sourceRows = [] } = {}) => {
  const setCalls = [];
  const findSourceRow = (id) =>
    sourceRows.find((row) => String(row.documentId || row.id) === String(id));
  const db = {
    setCalls,
    collection: jest.fn((name) => ({
      doc: jest.fn((id) => ({
        get: jest.fn(async () => {
          ...
          if (name === "BlogArticole") {
            const sourceRow = findSourceRow(id);
            return sourceRow ? makeDocSnap(id, sourceRow, true) : makeDocSnap(id, null, false);
          }
      ...
      get: jest.fn(async () =>
        makeQuerySnap(sourceRows.map((row) => makeDocSnap(row.documentId || row.id, row)))
      ),
    })),
  };
  return db;
...
describe("publicArticles", () => {
  ...
  it("loads published articles from materialized cache on first All page", async () => {
    const now = Date.now();
    const db = buildDb({
      manifest: {
        version: 1,
    ...
    expect(db.collection).toHaveBeenCalledWith("internalCaches");
  });
  ...
  it("derives detail related articles and next publish time from materialized cache", async () => {
    const now = Date.now();
    const db = buildDb({
      manifest: {
        version: 1,
        ...
        articlesPublic__chunk_0: {
          ...
          rows: [
            ...
            },
            {
              ...
              scheduledAtMs: now - 2_000,
              scheduledAtTs: new Date(now - 2_000).toISOString(),
              info: { ro: { nume: "Related cached", content: "<p>strip me</p>" } },
            },
          ],
        },
      },
      sourceRows: [
        {
          ...
          categorie: "Tarot",
          scheduledAtTs: new Date(now - 1_000).toISOString(),
          info: { ro: { nume: "Main full", content: "<p>full detail</p>" } },
        },
      ],
    });
    const { loadPublicArticleDetail } = await loadModuleWithDb(db);

    const payload = await loadPublicArticleDetail({ id: "doc-main", locale: "ro" });

    expect(payload.article?.info?.ro?.content).toBe("<p>full detail</p>");
    expect(payload.related.map((item) => item.documentId)).toEqual(["doc-related"]);
    expect(payload.related[0]?.info?.ro?.content).toBeUndefined();
    expect(payload.nextPublishAtMs).toBe(now + 30_000);
  });
  ...
  it("sorts by dataProgramata even when scheduledAtTs is stale", async () => {
    const now = Date.now();
    const db = buildDb({
      manifest: {
        version: 1,
      ...
      chunks: {
        articlesPublic__chunk_0: {
          ...
          rows: [
            {
              documentId: "doc-old-ts",
              id: "legacy-old-ts",
              scheduledAtMs: now + 86_400_000,
              scheduledAtTs: new Date(now + 86_400_000).toISOString(),
              dataProgramata: "01-01-2020",
              timpProgramat: "08:00",
              info: { ro: { nume: "Vechi programat corect" } },
            },
            {
              documentId: "doc-new",
              id: "legacy-new",
    ...
    const payload = await loadPublicArticles({ limit: 2, locale: "ro" });
    expect(payload.articles.map((item) => item.documentId)).toEqual(["doc-new", "doc-old-ts"]);
  });

  it("hides scheduled articles until publish time", async () => {
    const now = Date.now();
    const db = buildDb({
      manifest: {
        version: 1,
          ...
          rows: [
            {
              documentId: "doc-future",
              id: "legacy-future",
              scheduledAtMs: now + 7 * 86_400_000,
              scheduledAtTs: new Date(now + 7 * 86_400_000).toISOString(),
              dataProgramata: "31-12-2099",
              timpProgramat: "08:00",
              info: { ro: { nume: "Viitor" } },
            },
            {
    ...
    expect(payload.articles.map((item) => item.documentId)).toEqual(["doc-live"]);
  });
