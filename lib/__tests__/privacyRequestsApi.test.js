...
describe("POST /api/privacy-requests", () => {
  ...
  beforeEach(() => {
    process.env = { ...originalEnv, EMAIL_USER: "mailer@example.com", EMAIL_PASS: "test-password", PRIVACY_REQUEST_EMAIL: "privacy@example.com" };
  ...
  it("emails the privacy recipient and sends a confirmation", async () => {
    ...
    expect(sendMail.mock.calls[0][0]).toMatchObject({ to: "privacy@example.com", replyTo: "ana@example.com" });
