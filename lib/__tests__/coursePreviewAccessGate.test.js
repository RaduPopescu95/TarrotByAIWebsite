...
describe("applyCourseDetailContentGate", () => {
  test("keeps curriculum, notes, and contact for paid courses without access", () => {
    const locked = applyCourseDetailContentGate(
      toSafeCourse(
        "course-1",
        {
          ...
          notesContent: "Secret notes",
          contactContent: "Secret contact",
        },
        "ro",
    ...
    expect(locked.curriculumLessons).toEqual([
      expect.objectContaining({ id: "l1", title: "Lesson 1" }),
    ]);
    expect(locked.notesContent).toBe("Secret notes");
    expect(locked.contactContent).toBe("Secret contact");
  });
