...
describe("dashboardSession", () => {
  ...
  it("validates the fixed password shared by dashboard and administrare", () => {
    expect(verifyDashboardPassword("Cristina1994!")).toBe(true);
  ...
  it("uses the built-in session-secret fallback when server secrets are missing", () => {
    delete process.env.DASHBOARD_SESSION_SECRET;
    expect(() => createDashboardSession()).not.toThrow();
  });
});
