...
export default function NotificariManualeFields({
  ...
  // Adaugă la început, lângă celelalte useState
const [updateAvailable, setUpdateAvailable] = useState(false);
  ...
  const handleUploadData = () => {
    ...
    const data = {
      ...
      rusa: { nume: numeRusa, descriere: descriereRusa },
      tr: { nume: numeTr, descriere: descriereTr },
      ar: { nume: numeAr, descriere: descriereAr },
    ...
    <>
      <Grid container spacing={2} sx={{ padding: 1 }}>
        <>
          ...
          >
            <FormControlLabel
            sx={{ color: "white" }} // stil pe label (text)
  control={
    <Checkbox
    checked={updateAvailable}
    onChange={(event) => setUpdateAvailable(event.target.checked)}
    sx={{
      color: "white",          // Culoarea default (borderul când nu e bifat)
      "&.Mui-checked": {
        color: "white",        // Culoarea când e bifat
      },
    }}
  />
  
  }
  label="Update disponibil?"
/>
