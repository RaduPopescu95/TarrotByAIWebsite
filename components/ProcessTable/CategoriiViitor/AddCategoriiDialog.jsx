...
export default function AddCategoriiDialog(props) {
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      if (selectedImages.length > 0) {
        uploadImageServices(selectedImages, "", true).then((image) => {
          if (image && image.finalUri) {
            props.handleUploadService(
              name,
              title,
              metaDescription,
              tags,
      ...
      console.log("Error at handleSubmit add service...", err);
    }
  };
