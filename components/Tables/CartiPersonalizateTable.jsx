...
export default function CartiPersonalizateTable() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (dialogData?.image?.fileName) {
        const deletedRef = storageRef(
          storage,
          `images/Citire-Personalizata/${currentUser?.uid}/${dialogData.image.fileName}`
        );
        deleteObject(deletedRef).catch((error) => {
          console.log("Uh-oh, an error occurred! AT uploadImage DELETE...", error);
        });
      }
      ...
      handleShowDialog();
      handleDelete();
      await rebuildPublicTarotCacheAfterMutation("delete personalized card");
    } catch (error) {
    ...
    try {
      ...
      const updateData = db.map(async (item) => {
        if (item.id === dialogData.id) {
          ...
          await editData(data, "Citire-Personalizata", "Carti", dialogData.id);
          return data;
        } else {
      ...
      setDb([...updatedData]);
      handleShowDialog();
      await rebuildPublicTarotCacheAfterMutation("update personalized card");
    } catch (err) {
  ...
  const handleUpload = async (info, selectedImages) => {
    try {
      ...
      setShowSettings(!showSettings);
      await rebuildPublicTarotCacheAfterMutation("create personalized card");
    } catch (err) {
