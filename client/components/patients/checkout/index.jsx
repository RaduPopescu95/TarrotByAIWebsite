...
const Checkout = (props) => {
  ...
  const { currentUser, userData, selectedSlot, setSelectedSlot } = useAuth();
  
  // State pentru fiecare input
  const [categorii, setCategorii] = useState([]);
  const [categorie, setCategorie] = useState({});
  const [tipConsultatie, setTipConsultatie] = useState("");
  const [costConsultatie, setConstConsultatie] = useState("100");
  ...
  const handleUploadRezervare = async () => {
    ...
    console.log(data); // Afișează obiectul în consola
  
    try {
      await handleUploadFirestoreGeneral(data, "RezervariConsultatii");
      // Redirect în cazul succesului
  ...
  // Functia de submit pentru toate inputurile
