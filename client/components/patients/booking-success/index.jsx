...
const BookingSuccess = (props) => {
  ...
  const checkIfSessionExists = async (session_id) => {
    try {
      ...
      ); // Găsește documentul cu session_id corespunzător
    ...
    if (session_id) {
      const fetchSession = async () => {
        try {
          ...
          if (existingDocument) {
            console.log(
              `Session ID ${session_id} există deja în Firestore. Datele nu vor fi reîncărcate.`
            );
                ...
                <div className="card-body">
                  <div className="success-cont">
                    ...
                    currentUser.uid !== "AW8kjQIhAiaJM5q0QgGlOKpGF2j1" ? (
                      ...
                      <p>
                        Pe adresa de e-mail, precum și pe numărul de telefon, a
                        fost transmis link-ul de conectare. Conectați-vă la link
                        la{" "}
                        <strong>
                          {formatDateSlot(dateRezervari?.selectedSlot?.day)}-
                          {dateRezervari?.selectedSlot?.currentYear}
