...
const Invoices = () => {
  ...
  // descarca toate facturile individual iterare
  const handleDownloadAllInvoicesIndividually = async () => {
    try {
      ...
      for (const invoice of filteredInvoices) {
        try {
          // Fetch pentru obținerea URL-ului PDF al facturii
          const response = await fetch(
            `/api/get-invoice-pdf?invoiceId=${invoice.id}`
          );
          const data = await response.json();
          const pdfUrl = data.pdfUrl;

          if (pdfUrl) {
            // Deschide fiecare PDF într-o fereastră nouă
            window.open(pdfUrl, "_blank");
          } else {
            console.error(
              `Factura cu ID-ul ${invoice.id} nu are link PDF disponibil.`
            );
          }
        } catch (error) {
          console.error(
            `Eroare la descărcarea facturii cu ID-ul ${invoice.id}:`,
            error
          );
        }
      }
    } catch (error) {
      console.error("Eroare la descărcarea facturilor individuale:", error);
    } finally {
      setIsDownloading(false); // Oprește loaderul
    }
  };
