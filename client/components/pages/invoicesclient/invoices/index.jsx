...
const Invoices = () => {
  ...
  // Funcție pentru preluarea facturilor
  const fetchInvoices = async () => {
    if (!userData?.owner_uid) return;

    try {
      const response = await fetch(
        `/api/get-invoices-client?owner_uid=${userData?.owner_uid}`
      );
  ...
  // Funcție pentru descărcarea unei facturi individuale
  const handleDownloadInvoice = async (invoiceId) => {
    if (!userData?.owner_uid) return;

    try {
      const response = await fetch(
        `/api/get-invoice-pdf?invoiceId=${invoiceId}&owner_uid=${userData?.owner_uid}`
      );
      const data = await response.json();
  ...
  // Funcție pentru filtrarea facturilor pe baza textului de căutare și lunii selectate
  ...
  // Calcularea facturilor pentru pagina curentă
  const currentInvoices = filteredInvoices.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  // Generarea numerelor de pagină pentru paginare
  ...
  // Gestionare schimbare pagină
  const handlePageChange = (pageNumber) => {
    setCurrentPage(pageNumber);
  };

  // Inițializează preluarea facturilor la montarea componentului
  useEffect(() => {
    fetchInvoices();
  }, [userData?.owner_uid]);
  return (
